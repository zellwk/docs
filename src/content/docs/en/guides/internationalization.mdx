...
Configures whether or not the home URL (`/`) generated by `src/pages/astro.index` will redirect to `/<defaultLocale>`.
...
This is the **default** strategy. The strategy is applied to the pathname of the URLs.

#### `strategy: "domain"` (experimental)

This feature has some restrictions, which are listed below:
- the `output` option must be set to `"server"`;
- the `site` option is mandatory;
- there can't be any pre-rendered pages;
- the adapter feature [`functionPerRoute`](/en/reference/adapter-reference/#functionperroute) isn't supported; 

When a locale is mapped to domain, all the URLs won't have the language prefix. You map `fr` to `fr.example.com`, if you want to have a blog page to look like `fr.example.com/blog` instead of `example.com/fr/blog`. The localised folders be must in the `src/pages/` folder.

```js title="astro.config.mjs" ins={3-4} ins={11-18} ins="strategy: \"domains\"" 
import { defineConfig } from "astro/config"
export default defineConfig({
...
All page content files, including those for your `defaultLocale`, must exist in a localized folder within `src/pages/`
...
Make sure that your server proxy/hosting platform is able to provide such information. Failing to retrieve these headers will result in a 404 (status code) page.
