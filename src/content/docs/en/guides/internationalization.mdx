...
Use this strategy when you need to support multiple domains (e.g. `fr.example.com/blog`, `example.es/blog`). 

This routing strategy is available only for pages that are [rendered on-demand](/en/core-concepts/rendering-modes/#on-demand-rendered).

You must also specify the URL domain pattern to use in a `domains` configuration object. For any `locales` not configured in `domains`, a `/[locale]/` path for all non-default languages will be created.
...
Make sure that your server/hosting platform is able to provide such information. Failing to retrieve these headers will result in a 404 (status code) page.
